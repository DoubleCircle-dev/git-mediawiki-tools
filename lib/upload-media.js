...
async function syncMedia(opts = {}) {
  ...
  for (const p of candidates) {
    ...
    let res;
    try {
      res = await api.upload(name, fs.readFileSync(p), { comment });
    } catch (e) {
      res = { ok: false, why: e.message };
    }
    if (res.ok) {
      out.uploaded.push({ name, size, warnings: res.warnings });
      log(`  ⬆ 已上传  ${name} (${size})${res.warnings.length ? '  警告:' + res.warnings.join(',') : ''}`);
    } else {
      const hint = explainFailure(res.why);
      out.failed.push({ name, size, why: res.why, hint });
      log(`  ✗ 失败    ${name} (${size})  ${res.why}${hint ? `\n      ↳ ${hint}` : ''}`);
    }
  }
...
module.exports = {
  syncMedia, explainFailure, formatBestEffort,
