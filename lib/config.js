...
const DEFAULTS = {
  ...
  timeoutMs: 180000,
  // 删除页面处置（按内容模型）——见 bin/publish.js 与 README「页面删除」。
  //   modes 的键是 MediaWiki 内容模型（API prop=info 的 contentmodel：wikitext /
  //   Scribunto / sanitized-css / css / javascript / json / less …），值是模式名字符串
  //   或 { mode, text }：
  //     helper 不写占位，交给 git-mediawiki（wikitext → [[Category:Deleted]]）
  //     text   写入自定义正文（配 text；普通 wiki 页面常用，例如 {{需要删除}}）
  //     stub   清空为「该模型对应格式」的注释占位（文案取 stubNote）
  //     none   不改写，原样提交删除（服务端可能拒绝）
  //   未列出的模型用 defaultMode（缺省 helper）。
  deletion: {
    defaultMode: 'helper',
    text: '',
    stubNote: '本页已废弃：原内容已清空，请在线上删除本页。',
    modes: {
      Scribunto: 'stub',
      'sanitized-css': 'stub',
      css: 'stub',
...
function load() {
  ...
  // 5) 删除处置段合并默认（modes 逐模型合并，便于只覆盖个别模型）
  const fileDeletion = cfg.deletion || {};
  cfg.deletion = {
    ...DEFAULTS.deletion,
    ...fileDeletion,
    modes: { ...DEFAULTS.deletion.modes, ...(fileDeletion.modes || {}) },
  };
